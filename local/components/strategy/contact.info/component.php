...
if($_GET['id']>0){
	...
	while( $ob = $res->GetNextElement() ) {
		...
		if( !empty($arProps['REGION']['VALUE']) ){
			$region = Region::getById($arProps['REGION']['VALUE']);
			$arProps['REGION']['VALUE'] = $region['name_'.LANGUAGE_ID];
		}
		if( !empty($arProps['CITY']['VALUE']) ){
			$city = City::getById($arProps['CITY']['VALUE']);
			$arProps['CITY']['VALUE'] = $city['name_'.LANGUAGE_ID];
		}

		if( !empty($arProps['STREET']['VALUE']) ){
			$city = Street::getById($arProps['STREET']['VALUE']);
			$arProps['STREET']['VALUE'] = $city['name_'.LANGUAGE_ID];
		}
