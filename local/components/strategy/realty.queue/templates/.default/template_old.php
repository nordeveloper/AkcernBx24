                            ...
                            <? foreach ($arResult['NEW_LEADS'] as $arLead):?>
                                <label class="lead-item">
                                    <div class="check-item">
                                        <input  type="checkbox" name="leads[<?=$arLead['ID']?>]" value="<?=$arLead['TITLE']?>">
                                    </div>
                ...
                <? if(!empty($arResult['isAdmins'])):?>
                    <form method="post">
                        ...
                        <div class="form-group departments">
                            <p>Выберите подразделение что бы пропустить</p>
                            <? foreach ($arResult['DEPARTMENTS'] as $department): ?>
                                <label><input type="checkbox" name="department[<?=$department['XML_ID']?>]" value="<?=$department['XML_ID']?>">  <?=$department['NAME']?></label>
                            <? endforeach; ?>
                        </div>
                        <input type="submit" class="btn btn-success" name="submit" value="Прапустить">                
                    </form>
                <? endif ?>
            </div>

            <?/*
            <div class="tab-pane fade" id="leadsinwork" role="tabpanel" aria-labelledby="leadsinwork-tab">
                <p class="leads-title">Заявки в работе</p>
                <div class="lead-list">
                    <? if( !empty($arResult['LEADS_INPROCES']) ):?>
                    <? foreach ($arResult['LEADS_INPROCES'] as $arLeadP):?>
                        <div class="lead-item">
                            <div class="lead-wrapp">
                                <a target="_blank" class="lead-link" href="/crm/lead/details/<?=$arLeadP['ID']?>/"><?=$arLeadP['TITLE']?></a>
                                <div class="lead-date">Дата распределения։ <?=date('d.m.Y', strtotime($arLeadP['DATE_MODIFY']))?></div>
                            </div>
                        </div>
                    <? endforeach;?>
                    <? endif?>
...
<script>
    ...
    $(document).ready(function () {
        ...
        // $('#form-leads .btn').click(function() {
        //     if ($("#form-leads .lead-item input:checked").length < 1) {
        //         alert('Выберите лид для распределения');
        //         return false;
        //     }
        // });
