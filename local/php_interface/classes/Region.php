...
class Region
{
    public static function getList($arFilter=array()){
        ...
        $sql = "SELECT * FROM ak_regions ".$where." ORDER BY id asc";
        $dbres = $DB->query($sql);
        $arResult= array();
    ...
    public static function getById($id){
        ...
        if(intval($id)>0){
            $sql = "SELECT * FROM ak_regions WHERE id='$id' ORDER BY id asc";
            $dbres = $DB->query($sql);
    ...
    public static function Add($arData){
        ...
        if( !empty($arData['name_ru']) and !empty($arData['name_am']) ){
            ...
            $DB->StartTransaction();
            $ID = $DB->Insert('ak_regions', $arFields, $err_mess.__LINE__);
    ...
    public static function Update($id, $arData){
        ...
        if(intval($id)>0 and  !empty( trim($arData['name_am']) ) and  !empty(trim($arData['name_ru'])) ){
            ...
            $DB->StartTransaction();
            $DB->Update('ak_regions', $arFields, "WHERE id='".$id."'", $err_mess.__LINE__);
    ...
    public static function Remove($id){
        global $DB;
        if(intval($id)>0){
            $sql = "DELETE FROM ak_regions WHERE id='$id'";
            $res  = $DB->Query($sql);
            return $res;
        }
    }
    ...
    public static function count(){
        global $DB;
        $sql = "SELECT COUNT(id) as COUNT FROM ak_regions";
        $res =  $DB->Query($sql);
        $resC = $res->fetch();
        return $resC['COUNT'];
    }
